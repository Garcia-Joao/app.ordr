...
export default function SupplierDetailPage() {
  ...
  const supplier = useMemo(() => {
    return (
      suppliers.find(
        (item) => slugify(item.name) === wantedSlug || item.id === wantedSlug,
      ) ?? null
    );
  ...
  const selectedTable = useMemo(() => {
    ...
    return (
      supplier.priceTables.find((table) => table.id === selectedTableId) ??
      supplier.priceTables[0] ??
      null
    );
    ...
    try {
      ...
      setProducts(
        productResult.filter(
          (product) => !(product as { deletedAt?: unknown }).deletedAt,
        ),
      );
    } catch (err) {
      setError(
  ...
  useEffect(() => {
    ...
    if (
      !selectedTableId ||
      !supplier.priceTables.some((table) => table.id === selectedTableId)
    ) {
  ...
  async function refreshSupplier(nextSupplier?: Supplier) {
    ...
    if (current && slugify(current.name) !== wantedSlug) {
      router.replace(
        `/fornecedores/detalhe?slug=${encodeURIComponent(slugify(current.name))}`,
      );
    }
  }
  ...
  async function handleSaveSupplier() {
    ...
    try {
      const nextSupplier = await updateSupplier(
        supplier.id,
        supplierPayload(supplierForm),
      );
  ...
  async function handleDeactivateSupplier() {
    ...
    try {
      const nextSupplier = await deactivateSupplier(supplier.id);
      await refreshSupplier(nextSupplier);
  ...
  async function handleReactivateSupplier() {
    ...
    try {
      const nextSupplier = await reactivateSupplier(supplier.id);
      await refreshSupplier(nextSupplier);
      showSuccess("Fornecedor reativado.");
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Erro ao reativar fornecedor.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handlePermanentDeleteSupplier() {
    ...
    if (
      !window.confirm(
        `Excluir permanentemente o fornecedor ${supplier.name}? Essa ação não pode ser desfeita.`,
      )
    )
      return;
  ...
  async function handleDeleteTable(table: SupplierPriceTable) {
    ...
    try {
      const nextSupplier = await deleteSupplierPriceTable(
        supplier.id,
        table.id,
      );
      await refreshSupplier(nextSupplier);
  ...
  async function handleSaveItem() {
    ...
    try {
      const nextSupplier = editingItemId
        ? await updateSupplierPriceTableItem(
            supplier.id,
            selectedTable.id,
            editingItemId,
            payload,
          )
        : await createSupplierPriceTableItem(
            supplier.id,
            selectedTable.id,
            payload,
          );
  ...
  async function handleDeleteItem(item: SupplierPriceTableItem) {
    ...
    try {
      const nextSupplier = await deleteSupplierPriceTableItem(
        supplier.id,
        selectedTable.id,
        item.id,
      );
      ...
      <main className="min-h-screen bg-background p-6 text-foreground">
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-border bg-card p-8 text-center shadow-xl">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Volte para a listagem e selecione um fornecedor válido.
          </p>
          <Link
            href="/fornecedores"
            className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-primary px-4 py-3 text-sm font-black text-primary-foreground"
          >
  ...
  return (
    <main className="min-h-screen bg-background p-4 text-foreground sm:p-6">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
        <Link
          href="/fornecedores"
          className="inline-flex w-fit items-center gap-2 rounded-2xl border border-border bg-card px-4 py-2 text-sm font-black hover:bg-secondary"
        >
          <ArrowLeft className="h-4 w-4" />
          Fornecedores
        </Link>
        ...
        <section className="overflow-hidden rounded-[2rem] border border-border bg-card shadow-2xl">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <div className="mb-3 flex flex-wrap gap-2">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-black uppercase ${supplier.active ? "bg-emerald-500/15 text-emerald-300" : "bg-muted text-muted-foreground"}`}
                    >
          ...
          <div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <InfoCard
              label="Tabelas"
              value={String(supplier.priceTables.length)}
            />
            <InfoCard
              label="Itens"
              value={String(supplierItemsCount(supplier))}
            />
          </div>
          ...
          {(supplier.address || supplier.notes) && (
            <div className="grid gap-3 border-t border-border p-5 lg:grid-cols-2">
              {supplier.address && (
                <InfoPanel title="Endereço" text={supplier.address} />
              )}
              {supplier.notes && (
                <InfoPanel title="Observações" text={supplier.notes} />
              )}
            </div>
          )}
        </section>
        ...
        {(error || success) && (
          <div
            className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${error ? "border-red-500/30 bg-red-500/10 text-red-300" : "border-emerald-500/30 bg-emerald-500/10 text-emerald-300"}`}
          >
            {error || success}
          </div>
        )}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {table.items.length} item(ns)
                        </p>
                      </div>
                      <span
                        className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${table.active ? "bg-emerald-500/15 text-emerald-300" : "bg-muted text-muted-foreground"}`}
                      >
            ...
            {!selectedTable ? (
              <div className="flex min-h-[420px] items-center justify-center rounded-3xl border border-dashed border-border p-8 text-center">
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Crie uma tabela de preços para cadastrar os itens vendidos.
                  </p>
                </div>
              </div>
            ) : (
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {selectedTable.description || "Tabela sem descrição"} •
                      validade {formatDate(selectedTable.validFrom)} até{" "}
                      {formatDate(selectedTable.validUntil)}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl border border-red-500/30 px-4 py-2 text-sm font-black text-red-300 hover:bg-red-500/10"
                    >
                      <Trash2 className="h-4 w-4" />
                      Remover
                    </button>
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl bg-primary px-4 py-2 text-sm font-black text-primary-foreground hover:brightness-110"
                    >
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="inline-flex w-fit rounded-2xl border border-border bg-background p-1">
                    <button
                      ...
                      className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-black ${itemViewMode === "grid" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}
                    >
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-black ${itemViewMode === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}
                    >
      ...
      {modalKind === "table" && (
        <TableModal
          form={tableForm}
          saving={saving}
          editing={Boolean(editingTableId)}
          onClose={closeModal}
          onSave={handleSaveTable}
          onChange={(patch) =>
            setTableForm((current) => ({ ...current, ...patch }))
          }
        />
      )}
      {modalKind === "item" && (
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {item.product ? (
          <span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-black text-primary">
            {item.product.emoji ? `${item.product.emoji} ` : ""}
            {item.product.name}
          </span>
        ) : (
...
const inputClass =
  "rounded-2xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary";
...
function SupplierModal({
  ...
  return (
    <ModalShell
      ...
      description="Atualize dados visuais, categorias e contato."
      onClose={onClose}
      footer={
        <>
          <button
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field
          ...
          hint="Separe por vírgula. Ex: Bebidas, Alimentos, Limpeza"
        >
          <input
        ...
        <label className="flex items-center gap-3 rounded-2xl border border-border bg-background p-4 text-sm font-black md:col-span-2">
          <input
            type="checkbox"
            checked={form.active}
...
function TableModal({
  ...
  return (
    <ModalShell
      ...
      description="Use tabelas para separar cotações, validade ou condições comerciais."
      onClose={onClose}
      footer={
        <>
          <button
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Nome" className="md:col-span-2">
          <input
            ...
            onChange={(event) => onChange({ name: event.target.value })}
            className={inputClass}
            placeholder="Tabela padrão, cotação maio..."
          />
        </Field>
...
function ItemModal({
  ...
  return (
    <ModalShell
      ...
      description="Cadastre um item livre ou vincule a um produto do estoque."
      onClose={onClose}
      footer={
        <>
          <button
          ...
          <button
            ...
            disabled={
              saving ||
              !form.unitPrice ||
              (!form.itemName.trim() && !form.productId)
            }
            className="inline-flex items-center gap-2 rounded-2xl bg-primary px-4 py-3 text-sm font-black text-primary-foreground disabled:opacity-50"
          >
            {saving ? (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Produto do estoque" className="md:col-span-2">
          <select
            value={form.productId}
            onChange={(event) => onProductSelected(event.target.value)}
            className={inputClass}
          >
        ...
        <Field label="Unidade">
          <select
            value={form.unit}
            onChange={(event) =>
              onChange({ unit: event.target.value as StockUnit })
            }
            className={inputClass}
          >
            {UNITS.map((unit) => (
